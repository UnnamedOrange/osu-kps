...
protected:
	/// <summary>
	/// Get id of the thread the hook runs on. The thread is guaranteed to be created after the base class is constructed.
	/// </summary>
	/// <returns></returns>
...
public:
	_hook_base() :
		hook_thread(&_hook_base::thread_proc, this)
	{
...
/// <summary>
/// The keyboard hook. Call set_callback to register a callback (see keyboard_hook::callback_t). The callback function will run on an individual thread.
/// </summary>
...
/// <summary>
/// The mouse hook. Call set_callback to register a callback (see mouse_hook::callback_t). The callback function will run on an individual thread.
/// </summary>
...
public:
	...
	/// Returning true means to skip the key message.
	/// </summary>
	using callback_t = std::function<bool(UINT message, PMSLLHOOKSTRUCT ms, std::chrono::steady_clock::time_point time)>;
private:
	std::binary_semaphore bs_callback{ 1 };
	callback_t callback;
public:
