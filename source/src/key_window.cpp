...
void key_window::init_d2d() {
    ...
    if (FAILED(factory::d2d1()->CreateHwndRenderTarget(
            D2D1::RenderTargetProperties(), D2D1::HwndRenderTargetProperties(hwnd, D2D1::SizeU(cwidth(), cheight())),
...
void key_window::build_indep_resource() {
    ...
    pRenderTarget->CreateSolidColorBrush(cache.light_active_color, cache.active_brush.reset_and_get_address());
...
void key_window::build_scale_dep_resource() {
    ...
    {
        create_text_format(cache.theme_font_collection.get_family_names()[0].c_str(), cache.theme_font_collection.get(),
                           24.0 * x, cache.text_format_key_name.reset_and_get_address());
        ...
        create_text_format(cache.theme_font_collection.get_family_names()[0].c_str(), cache.theme_font_collection.get(),
                           16.0 * x, cache.text_format_key_name_small.reset_and_get_address());
        ...
        for (unsigned i = 0; i < crt_keys; i++) {
            ...
            // 最外层的框。
            {
                pRenderTarget->DrawRoundedRectangle(
                    draw_rounded_rect, i == keys.size() ? cache.active_brush : cache.theme_brush, stroke_width);
            }
        }
    }
