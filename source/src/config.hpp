...
public:
    ...
    void regulate() {
        ...
        key_monitor_implement(static_cast<kps::key_monitor_implement_type>(
            std::max(0, std::min(2, static_cast<int>(key_monitor_implement())))));
...
public:
    ...
    static std::u8string key_keyname(int total, int which) {
        ...
        return std::u8string(
            reinterpret_cast<const char8_t*>(("key." + std::to_string(total) + "k.k" + std::to_string(which)).c_str()));
    }
