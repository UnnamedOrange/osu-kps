    ...
    void OnKey(HWND hwnd, UINT vk, BOOL fDown, int cRepeat, UINT flags) {
        if (fDown && cRepeat == 1) {
            if (vk == VK_ESCAPE) {
                ...
                bool ok = true;
                switch (vk) {
                case VK_CONTROL: vk = VK_LCONTROL; break;
                case VK_SHIFT: vk = VK_LSHIFT; break;
                case VK_MENU: vk = VK_LMENU; break;
                default: ok = false;
                }
                if (ok) {
                    if (vk != VK_LSHIFT) {
...
public:
    key_window(config* cfg, keys_manager* k_manager) : cfg(cfg), k_manager(k_manager) {}
