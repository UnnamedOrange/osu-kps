...
public:
    ...
    void update(const Json::Value& json_template, bool is_base = true) {
        ...
        for (auto it = json_template.begin(); it != json_template.end(); it++) {
            ...
            if (!is_base) {
                ...
                case Json::ValueType::nullValue: value = Json::Value(Json::ValueType::nullValue); break;
                case Json::ValueType::booleanValue: value = value.asBool(); break;
                case Json::ValueType::intValue: value = value.asInt64(); break;
                case Json::ValueType::uintValue: value = value.asUInt64(); break;
                case Json::ValueType::realValue: value = value.asDouble(); break;
                default: // 其余无需转换。
                    break;
                }
            }
    ...
    void update(std::u8string_view json_content, bool is_base = true) {
        ...
        Json::String error;
        if (!reader->parse(reinterpret_cast<const char*>(json_content.data()),
                           reinterpret_cast<const char*>(json_content.data() + json_content.length()), &json_template,
                           &error) ||
            error.size())
    ...
    void update(const std::filesystem::path& json_file_path, bool is_base = true) {
        ...
        std::string str{std::istreambuf_iterator<char>(ifs), std::istreambuf_iterator<char>()}; // UTF-8
        update(std::u8string_view(reinterpret_cast<const char8_t*>(str.data())), is_base);
    }
...
public:
    ...
    value_t get_value(std::u8string_view key) const {
        ...
        switch (value.type()) {
        case Json::ValueType::nullValue: return std::nullopt;
        case Json::ValueType::booleanValue: return value.asBool();
        case Json::ValueType::intValue: return value.asInt64();
        case Json::ValueType::uintValue: return value.asUInt64();
        case Json::ValueType::realValue: return value.asDouble();
        case Json::ValueType::stringValue: return std::u8string(reinterpret_cast<const char8_t*>(value.asCString()));
        default: throw std::runtime_error("type not supported.");
        }
    }
