...
public:
    ...
    bool load_language(std::string_view json_content) {
        ...
        if (!reader->parse(reinterpret_cast<const char*>(json_content.data()),
                           reinterpret_cast<const char*>(json_content.data() + json_content.length()), &loaded_json,
                           &error) ||
            error.size())
            return false;
    ...
    bool load_language(const std::filesystem::path& path) {
        ...
        try {
            ...
            json_content = std::string(std::istreambuf_iterator<char>(ifs), std::istreambuf_iterator<char>());
        } catch (...) {
            return false;
        }
...
public:
    ...
    /// <returns>If the language does not exist in this multi_language object, returns false. Otherwise returns
    /// true.</returns>
