...
/// 在编码间进行转换时出错。
/// </summary>
class code_conv_error : public std::runtime_error {
...
private:
    static constexpr std::tuple<std::array<char8_t, 6>, size_t> convert_once(char32_t ch) {
        constexpr std::array<std::make_unsigned_t<char8_t>, 6> prefix{0, 0xC0, 0xE0, 0xF0, 0xF8, 0xFC};
        constexpr std::array<char32_t, 6> code_up{0x80, 0x800, 0x10000, 0x200000, 0x4000000, 0x80000000};
...
public:
    [[nodiscard]] static std::wstring convert(std::u8string_view src) {
        int length =
            MultiByteToWideChar(CP_UTF8, NULL, reinterpret_cast<LPCCH>(src.data()), int(src.length()), nullptr, NULL);
        std::wstring ret(length, 0);
        if (length != MultiByteToWideChar(CP_UTF8, NULL, reinterpret_cast<LPCCH>(src.data()), int(src.length()),
                                          ret.data(), int(ret.length())))
...
public:
    [[nodiscard]] static std::u8string convert(std::wstring_view src) {
        int length = WideCharToMultiByte(CP_UTF8, NULL, reinterpret_cast<LPCWCH>(src.data()), int(src.length()),
                                         nullptr, NULL, nullptr, FALSE);
        std::u8string ret(length, 0);
        if (length != WideCharToMultiByte(CP_UTF8, NULL, reinterpret_cast<LPCWCH>(src.data()), int(src.length()),
...
public:
    [[nodiscard]] static std::wstring convert(std::string_view src) {
        int length =
            MultiByteToWideChar(CP_ACP, NULL, reinterpret_cast<LPCCH>(src.data()), int(src.length()), nullptr, NULL);
        std::wstring ret(length, 0);
        if (length != MultiByteToWideChar(CP_ACP, NULL, reinterpret_cast<LPCCH>(src.data()), int(src.length()),
                                          ret.data(), int(ret.length())))
            throw code_conv_error("fail to MultiByteToWideChar.");
        return ret;
    }
...
public:
    [[nodiscard]] static std::string convert(std::wstring_view src) {
        int length = WideCharToMultiByte(CP_ACP, NULL, reinterpret_cast<LPCWCH>(src.data()), int(src.length()), nullptr,
                                         NULL, nullptr, FALSE);
        std::string ret(length, 0);
        if (length != WideCharToMultiByte(CP_ACP, NULL, reinterpret_cast<LPCWCH>(src.data()), int(src.length()),
                                          reinterpret_cast<LPSTR>(ret.data()), int(ret.length()), nullptr, FALSE))
            throw code_conv_error("fail to WideCharToMultiByte.");
        return ret;
    }
...
public:
    [[nodiscard]] static std::wstring convert(std::u32string_view src) {
        return code_conv<char8_t, wchar_t>::convert(code_conv<char32_t, char8_t>::convert(src));
    }
};
/// <summary>
...
public:
    [[nodiscard]] static std::u32string convert(std::wstring_view src) {
        return code_conv<char8_t, char32_t>::convert(code_conv<wchar_t, char8_t>::convert(src));
    }
};
#endif
